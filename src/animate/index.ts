// write it after study animateplus
...
type ElementsType<T extends HTMLElement = HTMLElement> = string | T[] | T;

type AnimateValue = string | number;

type CSSStyle = {
  [P in keyof CSSStyleDeclaration]?: [
    CSSStyleDeclaration[P] extends infer R ? R : AnimateValue,
    CSSStyleDeclaration[P] extends infer R ? R : AnimateValue,
  ];
};
...
interface Options<T extends HTMLElement = HTMLElement> extends CSSStyle, BaseOptions {
  elements: ElementsType<T>;
}
...
type RecomposeValue = (values: number[][], strings: string[], easing: number) => string;
...
const extractStrings: ExtractFuncType<string[]> = (value) => value.split(extractRegExp);
// 分割数字
...
const getElements = (elements: ElementsType) => {
  ...
  else if (typeof elements === 'string') return Array.from(document.querySelectorAll(elements));
...
const createAnimationKeyframes = (keyframes: CSSStyle): Keyframes[] =>
  Object.entries(keyframes).map(([property, values]) => {
    ...
    const stringValues = stringFormat(values as AnimateValue[]);
...
const getCurrentValue = (from: number, to: number, easing: number) => from + (to - from) * easing;
...
const animate = <T extends HTMLElement>(options: Options<T>): Promise<void> =>
  new Promise((resolve) => {
